...
void CameraWebServer::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Max open sockets: 4");
...
esp_err_t CameraWebServer::handler_(struct httpd_req *req) {
  if (this->mode_ == STREAM && this->running_) {
    ESP_LOGW(TAG, "STREAM: rejected duplicate client");
    httpd_resp_set_type(req, "text/plain");
    httpd_resp_set_status(req, "503 Busy");
    httpd_resp_sendstr(req, "Stream already active.\n");
    return ESP_OK;
  }
...
esp_err_t CameraWebServer::streaming_handler_(struct httpd_req *req) {
  ...
  while (res == ESP_OK && this->running_) {
    ...
    if (res == ESP_OK) {
      frames++;
      const uint32_t frame_time = millis() - last_frame;
      last_frame = millis();
      if (millis() - last_log_ms >= 5000) {
        last_log_ms = millis();
        ESP_LOGD(TAG, "MJPG: %" PRIu32 "B %" PRIu32 "ms (%.1ffps)", (uint32_t) image->get_data_length(), frame_time,
                 1000.0 / frame_time);
      }
    }
  }
