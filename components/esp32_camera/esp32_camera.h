...
typedef struct {
  ...
  struct timeval
      timestamp; // Timestamp since boot of the first DMA buffer of the frame
} camera_fb_t;
#else
...
enum ESP32CameraFrameSize {
  ...
  ESP32_CAMERA_SIZE_480X320,   // HVGA
  ESP32_CAMERA_SIZE_400X296,   // CIF
  ESP32_CAMERA_SIZE_640X480,   // VGA
  ESP32_CAMERA_SIZE_800X480,   // WVGA
...
public:
  ...
  /* public API (specific) */
  void
  add_image_callback(std::function<void(std::shared_ptr<CameraImage>)> &&f);
